...
public class MACDTests
{
    ...
    public void MACD()
    {
        var indicator = new MACD(IndicatorCapacity.Infinite);
        ...
        Assert.AreEqual("0.13", indicator.Values.Last().ToString("F2"));
        Assert.AreEqual("-0.11", indicator.Histogram.Last().ToString("F2"));
        Assert.AreEqual("0.25", indicator.Signal.Last().ToString("F2"));
    }
}
